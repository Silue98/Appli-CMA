...
export default defineEventHandler(async (event) => {
  ...
  if (!body.evenementId) throw createError({ statusCode: 400, message: 'evenementId requis' })

  const settings = await prisma.emailSettings.findFirst()
  if (!settings?.autoEvenement) return { success: false, message: 'Envoi automatique désactivé' }

  const evenement = await prisma.evenement.findUnique({ where: { id: Number(body.evenementId) } })
  if (!evenement) throw createError({ statusCode: 404, message: 'Événement non trouvé' })
  ...
  const dateFormatee = new Date(evenement.dateDebut).toLocaleDateString('fr-FR', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })
  ...
  const content = `
    ...
    <p>Nous comptons sur votre présence. Que Dieu vous bénisse ! 🙏</p>
  `

  const html = emailTemplate(`Nouvel événement : ${evenement.titre}`, content)

  try {
    await sendMail({ to: emails, subject: `⛪ CMA — ${evenement.titre}`, html })
    await prisma.emailLog.create({ data: { sujet: `Nouvel événement : ${evenement.titre}`, destinataires: emails.length, type: 'AUTO_EVENEMENT', statut: 'ENVOYE' } })
    return { success: true, envoyes: emails.length }
  } catch (err: any) {
    await prisma.emailLog.create({ data: { sujet: `Nouvel événement : ${evenement.titre}`, destinataires: emails.length, type: 'AUTO_EVENEMENT', statut: 'ERREUR', erreur: err.message } })
