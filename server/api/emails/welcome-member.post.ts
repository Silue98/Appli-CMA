...
export default defineEventHandler(async (event) => {
  ...
  if (!body.membreId) {
    throw createError({ statusCode: 400, message: 'membreId requis' })
  }

  // Vérifier si l'envoi auto est activé
  const settings = await prisma.emailSettings.findFirst()
  if (!settings?.welcomeEnabled) {
    return { success: false, message: 'Email de bienvenue désactivé' }
  }

  const membre = await prisma.membre.findUnique({
    where: { id: Number(body.membreId) }
  })

  if (!membre || !membre.email) {
    return { success: false, message: 'Membre sans email' }
  }
  ...
  const content = `
    ...
    <div style="background:#f0fdf4; border-left:4px solid #15803d; padding:15px; margin:15px 0; border-radius:5px;">
      ...
      <p style="margin:5px 0;">📋 Numéro de membre : <strong>${membre.numeroMembre || 'À définir'}</strong></p>
    </div>
  ...
  try {
    await sendMail({
      to: membre.email,
      subject: `⛪ Bienvenue chez CMA DOKUI1, ${membre.prenom} !`,
      html
    })

    await prisma.emailLog.create({
      data: {
        sujet: `Bienvenue : ${membre.prenom} ${membre.nom}`,
        destinataires: 1,
        type: 'BIENVENUE',
        statut: 'ENVOYE'
      }
